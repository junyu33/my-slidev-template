...
而之前已经算出 $\omega^3=-1$，6 的非平凡因子只有 2 和 3，$\omega$ 不是本原的 2 次或 3 次单位根，因此只能是 6 次。
...
$$
a \times a = a^2, \quad (b\omega^2 w)(cw^2)=vbc \omega^2, \quad (c\omega^4w^2)(bw)=vbc \omega^4
$$
...
(p_0^(1), p_1^(1), p_2^(1), p_3^(1), p_0^(2), p_1^(2), p_2^(2), p_3^(2))
    ← MUL_FP_8WAY(                      // 竖着看
...
有了四位卷积的内核，我们就可以实现 `MUL_FP_8WAY` 了。我们假设 8-lane 输入 $\alpha^{(j)}, \beta^{(j)}$ 被分成了两半 $\alpha^{(j)} = \alpha_{L}^{(j)} + R\alpha_{H}^{(j)}, R=2^{48}$，则直接套用 Karatsuba 公式：
