...
那么在这一轮广播之后，Alice 将会拿到 $(bP, bQ, cP, cQ)$，加上 Alice 本身也知道 $(a, P, Q)$，她的任务就是要想办法从 $(bP, bQ, cP, cQ)$ 中组合出一种关于 $bc$ 的量——
...
公开椭圆曲线 $E(\mathbb{F}_p)$，两个位于 $E$ 上且线性无关的点 $P,Q$.
...
特别地，如果点 $P \in E[m]$（即 $P$ 满足 $mP = \mathcal{O}$），那么 $\text{div}(f_P) = m[P] - m[\mathcal{O}]$，符合之后要提到的 weil pairing 的条件。
...
> 
> 假设 $b_{n-i-1} = 0$，则不走 if 分支，我们实际的运算为 $f' = f^2 \cdot g_{m_iP, m_iP}, T' = 2m_iP$。则此时新的 $\text{div}(f')$：
> 
> 
...
```python
    def tate(self, Q, S) -> int:
        ...
        if q % l != 1:
            raise ValueError('q must be 1 mod l')
